...
def api_post(path: str, json_body: dict | None = None) -> dict:
    r = requests.post(f"{BASE_URL}{path}", json=json_body, timeout=60)
    r.raise_for_status()
    return r.json()
...
class ImagePreview(Static):
    ...
    def _fetch(self, post_id: int) -> None:
        try:
            resp = requests.get(f"{BASE_URL}/api/v1/library/{post_id}/file", timeout=20)
...
class LibraryTab(Widget):
    ...
    def _do_download(self, p: dict) -> None:
        ...
        try:
            resp = requests.get(f"{BASE_URL}/api/v1/library/{post_id}/file", timeout=60)
            resp.raise_for_status()
...
class JobsTab(Widget):
    ...
    def _submit(self, payload: dict) -> None:
        ...
        self._stream(tags, payload.get("limit", 20), payload.get("sources", []),
                     author, payload.get("source_tags", {}), rk)
    ...
    def action_cancel_job(self) -> None:
        ...
        if job_id.startswith("stream-"):
            ...
            if s and s["active"]:
                ...
                if resp is not None:
                    try:
                        resp.close()  # closes the underlying socket for requests.Response
                    except Exception:
                        pass
        else:
            self._cancel(job_id)
    ...
    def _stream(self, tags: list[str], limit: int, sources: list[str],
        ...
        try:
            # requests streams via stream=True + iter_lines()
            resp = requests.get(
            ...
            if s["cancelled"]:
                ...
                resp.raise_for_status()
                # iter_lines() yields decoded str lines (chunk_size=None = default)
